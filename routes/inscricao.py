...
def createInscricao(corrida_id):
    ...
    if not all(k in data for k in ('atleta_id', 'categoria_id', 'numero')):
        return jsonify({"error": "Campos obrigatórios: atleta_id, categoria_id, numero"}), 400

    try:
        cursor = mysql.connection.cursor()
        cursor.execute("INSERT INTO Inscricao (atleta_id, corrida_id, categoria_id, numero, status) VALUES (%s, %s, %s, %s, 'ATIVA')",
                       (data['atleta_id'], corrida_id, data['categoria_id'], data['numero']))
